# Requirements Archive: v3.2 AI LessonAgent 起草闭环
...
- [x] **AGENT-01**: LessonAgent 暴露一组 Zod 校验的 typed tools，所有输入输出在边界处被校验，非法 payload 被拒绝。
- [x] **AGENT-02**: Agent 工具只能通过 DAL / Command Bus 读写数据，不能直连数据库、不能访问 provider key、不能执行任意代码。
...
- [x] **REVIEW-02**: 教师能逐项或整体编辑 AI 起草的步骤后再决定去留。
- [x] **REVIEW-03**: 教师能接受 AI 起草并使其进入既有发布链路，或丢弃起草且不影响原课时。
- [x] **REVIEW-04**: 审校界面对齐 Stitch 项目 `5322129002350954765` 与 `DESIGN.md`（Lexend、无 1px 分隔线、tonal surface、glass/gradient CTA）。
...
Every v3.2 requirement maps to a build phase. Coverage: 18/18. 其中 6 项（AGENT-03、DRAFT-01、DRAFT-02、DRAFT-03、REVIEW-01、REVIEW-03）的端到端生产路径由 **Phase 66** 收尾打通（run→persist 接缝、教师触发入口、accept/discard 经 Command Bus），并于 2026-06-02 通过 Phase 66 验证后计为 Complete（端到端生成由 mock-provider 测试 212/212 覆盖；旗标门控触发器可见/隐藏经 Playwright 验证）。
...
| AGENT-01 | Phase 62 | Complete |
| AGENT-02 | Phase 62 | Complete |
| AGENT-03 | Phase 62 → 66 | Complete |
| AGENT-04 | Phase 62 | Complete |
| DRAFT-01 | Phase 63 → 66 | Complete |
| DRAFT-02 | Phase 63 → 66 | Complete |
| DRAFT-03 | Phase 63 → 66 | Complete |
| REVIEW-01 | Phase 64 → 66 | Complete |
| REVIEW-02 | Phase 64 | Complete |
| REVIEW-03 | Phase 64 → 66 | Complete |
| REVIEW-04 | Phase 64 | Complete |
