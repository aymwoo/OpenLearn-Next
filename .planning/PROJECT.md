...
- `v1.3 Teaching Orchestration & Classroom Intelligence` 已归档；归档事实以 `.planning/MILESTONES.md` 与 `.planning/milestones/v1.3-MILESTONE-AUDIT.md` 为准。
- Phase 14 的 `COURSE-04`~`COURSE-06` 短 scope 已完成；下一步待从 `COURSE-07`、Phase 15 与其余 known gaps 中选择 follow-up。
- 刚完成的短 scope 只覆盖 `COURSE-04`~`COURSE-06`，即课程生命周期、删除 guardrail 与班级关联。
- `COURSE-07`、Phase 15 与 `AUTH-01`~`AUTH-06`、`DATA-01`~`DATA-05`、`CLASS-05` 继续后置。
- 在 scope 最终确认前，不预设新的 milestone 名称、日期或 phase。
...
### Known gaps after v1.3 close

- [x] `COURSE-04` 已在 `14-01` 中完成：教师可以 publish、unpublish、archive 课程，并收紧 archived 课程在相邻教师流程中的可见性。
- [x] `COURSE-05` 已在 `14-03` 中完成：课程删除通过显式确认路径执行，并在被阻止时提供页内结构化反馈。
- [x] `COURSE-06` 已在 `14-02` 中完成：教师可以在课程详情页内按 school scope 关联或移除班级，且不触及 student enrollment。
- [ ] `COURSE-07` ~ `COURSE-09` 继续保留为后置 carry-over backlog。
...
- [x] Phase 14 的 `COURSE-04` ~ `COURSE-06` 短 scope 已完成；`COURSE-07` 继续后置，不并入本轮实现。
- [ ] Phase 15 与 `COURSE-08` ~ `COURSE-09` 继续作为后续批量导入 backlog，不提前并入当前候选范围。
...
这意味着“可运行的课堂闭环基础”已经存在，但 live planning 当前不再有 active milestone implementation。Phase 14 的 `/teacher/courses` 短 scope 现已全部完成，其中 `14-01 / COURSE-04`、`14-02 / COURSE-06` 与 `14-03 / COURSE-05` 均已收口。下一步待从 `COURSE-07`、Phase 15，以及 `AUTH`、`DATA` 与 `CLASS-05` 中选择 follow-up。
...
---
*Last updated: 2026-05-15 after completing the Phase 14 short-scope follow-up on course lifecycle, delete guardrails, and class association*
