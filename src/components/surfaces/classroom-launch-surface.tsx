...
import { ArrowRight, Clock3, PlayCircle, RadioTower, UsersRound } from 'lucide-react'
...
export function ClassroomLaunchSurface({ consoleData }: ClassroomLaunchSurfaceProps) {
          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 text-sm leading-7 text-on-primary/85 sm:text-base sm:leading-8">
                先确认已发布课时与班级名单，再进入运行台。若当前已有 live classroom，可从右侧次级恢复区继续上课，但不会压过新开课堂主动作。
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-3 xl:w-[28rem]">
              <HeroMetric icon={<PlayCircle className="size-4" aria-hidden />} label="可开课时" value={String(consoleData.publishedLessons.length)} detail="仅展示可直接启动的已发布课时" />
              <HeroMetric icon={<UsersRound className="size-4" aria-hidden />} label="可用班级" value={String(classCount)} detail="已绑定可选名册" />
              <HeroMetric icon={<RadioTower className="size-4" aria-hidden />} label="进行中的课堂" value={String(consoleData.liveSessions.length)} detail="继续上课保持次级呈现" />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="space-y-4">
          <Card className="bg-surface-container-low p-5 sm:p-6">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm text-on-surface-variant">继续正在运行的课堂</p>
                <h2 className="mt-2 text-xl font-semibold text-on-surface">恢复 live classroom</h2>
              </div>
            ...
            <p className="mt-3 text-sm leading-6 text-on-surface-variant">
              已有 live classroom 时，这里会保留继续上课入口与当前状态摘要；如果没有正在运行的课堂，也会明确提示你直接开始新课堂。
            </p>

            <div className="mt-4 grid gap-3">
              {consoleData.liveSessions.length > 0 ? (
                consoleData.liveSessions.map((session) => (
                  <article key={session.id} className="rounded-[1.4rem] bg-surface-container-lowest p-4 shadow-ambient">
                    ...
                    <h3 className="mt-3 text-base font-semibold text-on-surface">{session.lessonTitle}</h3>
                    <p className="mt-1 text-sm text-on-surface-variant">{session.className}</p>
                    ...
                    <Button asChild variant="secondary" className="mt-4 w-full justify-center gap-2 text-sm">
                ...
                <article className="rounded-[1.4rem] bg-surface-container-lowest p-4 shadow-ambient">
                  <p className="text-sm font-medium text-on-surface">当前没有正在运行的课堂</p>
          ...
          <Card className="bg-surface-container-low p-5 sm:p-6">
            ...
            <div className="mt-4 grid gap-3">
              <InfoBlock title="先确认已发布课时" body="新课堂只会读取已发布版本，避免未定稿内容直接进入学生端。" />
              <InfoBlock title="名册会直接带入运行台" body="选定班级后，运行台会继续负责在线状态、锁定模式与步骤切换。" />
            </div>
          </Card>
        </div>
      </section>
