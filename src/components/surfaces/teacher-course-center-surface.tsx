...
export function TeacherCourseCenterSurface({ data }: TeacherCourseCenterSurfaceProps) {
  const archivedHref = data.includeArchived ? "/teacher/courses" : "/teacher/courses?archived=1";
      ...
      <section className="rounded-[var(--radius-shell)] bg-surface-container-low p-6 shadow-ambient sm:p-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-3xl">
            ...
            <h1 className="text-[2.25rem] font-semibold leading-tight tracking-[-0.02em] sm:text-[3rem]">我的课程总览</h1>
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-3">
          <CourseCenterMetric label="当前课程数" value={`${data.courses.length} 门`} />
          <CourseCenterMetric label="默认视图" value={data.includeArchived ? "含已归档" : "仅活跃课程"} />
      ...
      <section className="rounded-[var(--radius-shell)] bg-surface-container-low p-5 shadow-ambient sm:p-6">
        <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold">按课程阶段与最近更新组织</h2>
          </div>
        ...
        {data.courses.length > 0 ? (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {data.courses.map((course) => {
              ...
              return (
                <Card key={course.id} className="min-h-64 bg-surface-container-lowest p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div className="space-y-2">
                      <Badge variant="default">{course.subject}</Badge>
                      <div>{/* status badge rendering */}
                  ...
                  <h3 className="mt-5 text-2xl font-semibold text-on-surface">{course.title}</h3>
                  <p className="mt-3 text-sm leading-7 text-on-surface-variant">
                    {course.classLabels.length > 0 ? `关联班级：${course.classLabels.join("、")}` : "尚未关联班级，后续可继续配置。"}
                  </p>

                  <div className="mt-5 grid gap-3 sm:grid-cols-2">
                    <CourseCardMeta icon={<Layers3 className="size-4" />} label="课时数" value={`${course.lessonCount} 个`} />
                    <CourseCardMeta icon={<Users className="size-4" />} label="学生数" value={`${course.enrollmentCount} 名`} />
                    <CourseCardMeta icon={<GraduationCap className="size-4" />} label="年级" value={course.grade} />
                    <CourseCardMeta icon={<Clock3 className="size-4" />} label="最近更新" value={formatDateTime(course.updatedAt)} />
                  </div>

                  <div className="mt-6 flex items-center justify-between gap-3">
                    <span className="text-sm text-on-surface-variant">先查看课程详情，再进入课时管理</span>
                    <Button asChild variant="tertiary" className="min-h-10 px-0 text-sm">
                      <Link href={`/teacher/courses/${course.id}`}>查看课程详情</Link>
                    </Button>
                  </div>
                </Card>
              );
          ...
          <div className="rounded-[calc(var(--radius-shell)-0.75rem)] bg-surface-container-lowest p-8 text-center shadow-ambient">
            ...
            <h3 className="mt-5 text-2xl font-semibold">当前还没有可显示的课程</h3>
            <p className="mt-3 text-sm leading-7 text-on-surface-variant">
...
function CourseCenterMetric({ label, value }: { label: string; value: string }) {
  return (
...
function CourseCardMeta({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
