...
import { invalidateScheduleAssistantTags } from "@/features/schedule/shared/cache";
...
export async function createScheduleAssistantProposalAction(input: FormData | Record<string, unknown>): Promise<ActionResult<unknown>> {
  ...
  try {
    ...
    invalidateScheduleAssistantTags(updateTag, { proposalId: proposal.id, schoolId: proposal.schoolId });
...
export async function approveScheduleAssistantProposalAction(input: { proposalId: string }): Promise<ActionResult<unknown>> {
  try {
    ...
    invalidateScheduleAssistantTags(updateTag, { proposalId: proposal.id });
    return { ok: true, data: proposal };
  } catch (error) {
    return handleError(error);
  }
