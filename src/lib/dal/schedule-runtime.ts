...
import { ClassDailyAgendaCardDTOSchema, TeacherDailyAgendaCardDTOSchema, TeacherDailyAgendaDTOSchema } from "@/lib/dto/schedule";
...
function buildTeacherCard(args: {
  ...
  return TeacherDailyAgendaCardDTOSchema.parse({
    ...
    overrideSummary,
    lessonLink: args.entry.lessonId ? { lessonId: args.entry.lessonId, lessonTitle: args.courseTitle } : null,
  });
}
...
async function getCachedTeacherDailyAgenda(actorId: string, schoolIds: string[], requestedDate: string) {
  ...
  cacheLife("minutes");
  cacheTag(cacheTags.teacherScheduleAgenda(actorId, requestedDate));
...
async function getCachedClassAgenda(classId: string, schoolIds: string[], requestedDate: string) {
  ...
  cacheLife("minutes");
  cacheTag(cacheTags.classScheduleAgenda(classId, requestedDate));
