# Requirements Archive: v3.0 AI Native Educational OS Upgrade
...
- [x] **CMD-01**: 平台调用方可以提交带有 `id`、`type`、`actor`、`scope`、`payload`、`correlation metadata` 的 `PlatformCommand`。
- [x] **CMD-02**: 系统会对每个 command 统一执行 `validate -> authorize -> execute -> record result` 的 pipeline。
- [x] **CMD-03**: 插件生命周期核心动作通过 Command Bus v1 执行，而不是继续直连旧 plugin action / service seam。
- [x] **CMD-04**: Command Bus 会把每次执行写入 durable command ledger，并保留 success / failure summary。
- [x] **CMD-05**: `install`、`enable`、`disable`、`retry` 等重复敏感命令支持 idempotency / dedupe key。
...
- [x] **EVNT-01**: command 成功后会产生 typed platform event，与 command envelope 明确分离。
- [x] **EVNT-02**: 平台事件会写入 durable event outbox / ledger，并关联 `commandId`、`correlationId`、`causationId`。
- [x] **EVNT-03**: 插件、审计、分析和 future workflow / agent 订阅者可以消费 platform events，而不依赖 classroom runtime transport bus。
- [x] **EVNT-04**: 系统可以把 platform events 桥接到 in-process、Redis、WebSocket delivery adapters，但不改变 SQLite truth ownership。
- [x] **EVNT-05**: command、event、task、audit 共享统一 correlation metadata。
- [x] **EVNT-06**: command handlers 返回 invalidation intent，使入口层能统一 `updateTag()` / `revalidateTag()`。
- [x] **EVNT-07**: 平台维护者可以在最小 operator surface 查看 command / event execution summary 与 failure attribution。
...
| CMD-01 | Phase 51 | Complete |
| CMD-02 | Phase 51 | Complete |
| CMD-03 | Phase 51 | Complete |
| CMD-04 | Phase 51 | Complete |
| CMD-05 | Phase 51 | Complete |
...
| EVNT-01 | Phase 53 | Complete |
| EVNT-02 | Phase 53 | Complete |
| EVNT-03 | Phase 53 | Complete |
| EVNT-04 | Phase 53 | Complete |
| EVNT-05 | Phase 53 | Complete |
| EVNT-06 | Phase 53 | Complete |
| EVNT-07 | Phase 53 | Complete |
...
*Last updated: 2026-05-23 after v3.0 milestone audit sync*
