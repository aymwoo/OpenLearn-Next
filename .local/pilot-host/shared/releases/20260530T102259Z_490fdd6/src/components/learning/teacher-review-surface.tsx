...
import type {
  QuizAttemptDTO,
  TaskAttemptDTO,
  TeacherLessonReviewDTO,
  TeacherReviewFilter,
  TeacherStudentReviewDTO,
...
function attemptText(attempt: TaskAttemptDTO) {
  const payload = attempt.payload as
    | { text?: string; answer?: string; body?: string }
    | string
    | null;
...
function quizOutcomeText(attempt: QuizAttemptDTO) {
  const outcome = attempt.outcome as {
...
function StudentDetail({ student }: { student: TeacherStudentReviewDTO }) {
  ...
  const feedbackTargets = [
    ...student.latestTaskSubmissions.map((attempt) => ({
      label: "任务反馈",
      targetType: "task_submission" as const,
      targetId: attempt.id,
      feedback: attempt.feedback,
    })),
    ...student.latestQuizAttempts.map((attempt) => ({
      label: "测验反馈",
    ...
    <div className="space-y-5">
      ...
      >
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Badge
            variant={student.needsFeedback ? "default" : "success"}
            className={
              student.needsFeedback ? "bg-[#fff3cd] text-[#856404]" : undefined
            }
          >
        ...
        <p className="mt-2 text-sm leading-6 text-on-surface-variant">
          当前已完成 {completed}/{total}{" "}
          个环节，可继续针对任务与测验统一填写反馈。
        </p>
      </div>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        >
          <div className="flex items-center justify-between gap-3">
            ...
            <Badge
              variant={latestTask?.feedback ? "success" : "default"}
              className={
                !latestTask?.feedback && latestTask
          ...
          {latestTask ? (
            <div className="mt-3 text-sm leading-6 text-on-surface-variant">
              <p>
                第 {latestTask.attemptNo} 次尝试 ·{" "}
                {formatTime(latestTask.createdAt)}
              </p>
        ...
        >
          <div className="flex items-center justify-between gap-3">
            ...
            <Badge
              variant={latestQuiz?.feedback ? "success" : "default"}
              className={
                !latestQuiz?.feedback && latestQuiz
                  ? "bg-[#fff3cd] text-[#856404]"
                  : undefined
              }
            >
          ...
          {latestQuiz ? (
            <div className="mt-3 text-sm leading-6 text-on-surface-variant">
              <p>
                {quizOutcomeText(latestQuiz)} · 第 {latestQuiz.attemptNo} 次尝试
              </p>
              <p className="mt-2">
                反馈状态：{latestQuiz.feedback ? "已反馈" : "待反馈"}
              </p>
            </div>
          ) : (
            <p className="mt-3 text-sm leading-6 text-on-surface-variant">
            ...
            {feedbackTargets.map((target) => (
              ...
              >
                ...
                <FeedbackComposer
                  ...
                  latestFeedback={target.feedback}
                />
              </div>
            ))}
          </div>
          ...
          {[...student.taskSubmissionHistory, ...student.quizAttemptHistory]
            ...
            .map((attempt) => (
              ...
              >
                ...
                {attempt.isLatest ? "最新" : "历史记录"}
              </article>
            ))}
          {student.taskSubmissionHistory.length +
            student.quizAttemptHistory.length ===
          0 ? (
            <p
              className={cn(
                teacherSurfaceRhythm.cardInset,
                "p-4 text-sm text-on-surface-variant",
              )}
            >
              第 1 次尝试会在学生提交后出现在这里。
            </p>
...
export function TeacherReviewSurface({
  review,
  selectedStudentId,
  filter = "all",
}: TeacherReviewSurfaceProps) {
  const activeFilter = review?.filter ?? filter;
  const selectedStudent =
    review?.students.find(
      (student) => student.studentId === selectedStudentId,
    ) ??
    review?.students[0] ??
    null;
  const filterBadgeClass =
    "bg-surface-container-lowest text-on-surface-variant shadow-none hover:bg-surface-container-high";
  const evaluatedCount = review
    ? review.students.length - review.overview.needsFeedbackCount
    : 0;
    ...
    >
        ...
        >
          <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
            ...
            <div className="grid gap-3 sm:grid-cols-3 xl:w-[420px]">
              <HeroMetric
                label="班级学生"
                value={String(review?.students.length ?? 0)}
              />
              <HeroMetric
                label="已评估"
                value={String(Math.max(evaluatedCount, 0))}
              />
              <HeroMetric
                label="待反馈"
                value={String(review?.overview.needsFeedbackCount ?? 0)}
              />
            </div>
          </div>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <Card className="bg-surface-container-lowest">
          <UsersRound
            className="mb-4 size-6 text-on-surface-variant"
            aria-hidden
          />
          <p className="text-sm text-on-surface-variant">未开始</p>
          <p className="mt-2 text-2xl font-semibold">
            {review?.overview.notStartedCount ?? 0}
          </p>
        </Card>
        <Card className="bg-surface-container-lowest">
          ...
          <p className="mt-2 text-2xl font-semibold">
            {review?.overview.inProgressCount ?? 0}
          </p>
        </Card>
        <Card className="bg-surface-container-lowest">
          ...
          <p className="mt-2 text-2xl font-semibold">
            {review?.overview.completedCount ?? 0}
          </p>
        </Card>
        <Card className="bg-surface-container-lowest">
          ...
          <p className="mt-2 text-2xl font-semibold">
            {review?.overview.needsFeedbackCount ?? 0}
          </p>
        </Card>
      </section>
          ...
          <div className={cn(teacherSurfaceRhythm.cardInset, "p-4")}>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {filters.map((item) => {
                ...
                const href = review
                  ? `/teacher/review?lessonId=${encodeURIComponent(review.lessonId)}&filter=${item.value}`
                  : `/teacher/review?filter=${item.value}`;
            ...
            {review && review.students.length > 0 ? (
              review.students.map((student) => {
                const selected =
                  ...
                  (!selectedStudentId &&
                    student.studentId === review.students[0]?.studentId);
                return (
                  <Link
                    ...
                    className={cn(
                      teacherSurfaceRhythm.card,
                      selected
                        ? "block bg-surface-container-lowest p-4 shadow-ambient"
                        : "block bg-surface-container-lowest/75 p-4",
                    )}
                  >
                    <div className="flex items-center justify-between gap-3">
          ...
          {selectedStudent ? (
            ...
            <div>
              ...
              <p className="mt-4 leading-8 text-on-surface-variant">
                详情区域会优先呈现最近提交、测验结果和反馈状态，适合批量评价场景快速切换。
              </p>
              <p
                className={cn(
                  teacherSurfaceRhythm.card,
                  "mt-6 bg-surface-container-low p-5 text-sm leading-6 text-on-surface-variant",
                )}
              >
                还没有提交学习证据
              </p>
...
function HeroMetric({ label, value }: { label: string; value: string }) {
  return (
    <div
      className={cn(
        teacherSurfaceRhythm.card,
        "bg-white/12 px-4 py-4 backdrop-blur-sm",
      )}
    >
