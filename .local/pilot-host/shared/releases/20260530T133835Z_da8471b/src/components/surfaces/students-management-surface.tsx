...
export function StudentsManagementSurface() {
  return (
    <div
      className={cn(
        `${surfaceWidths.workspace} flex flex-col pb-12 pt-3`,
        ...
        >
          <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
            <div>
              ...
              <h1 className="mt-4 text-[2.4rem] font-semibold tracking-[-0.02em] text-white">
                查看并管理所有班级的学生信息
              </h1>
              <p className="mt-3 text-base leading-8 text-on-primary/85">
                页面参考 Stitch
                的学生管理屏幕，保留当前静态样例数据，只优化筛选栏、列表布局和右侧统计卡。
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              <Button
                variant="secondary"
                className="gap-2 bg-white/92 text-base text-primary shadow-none hover:bg-white"
              >
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]">
            ...
            <div className="grid gap-3 lg:grid-cols-[repeat(3,minmax(0,1fr))_minmax(0,1.1fr)]">
              ...
              <div
                className={cn(
                  ...
                  "flex min-h-[132px] flex-col justify-between p-4",
                )}
              >
          ...
          <div className={teacherSurfaceRhythm.section}>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-wrap gap-3 text-sm">
                ...
                <Button
                  variant="secondary"
                  className="min-h-10 bg-error-container px-4 text-sm text-on-error-container hover:bg-error-container/90"
                >
                  批量删除
                </Button>
              </div>
            ...
            <div className="mt-5 grid gap-3">
              {students.map((student) => (
                <article
                  ...
                  className={cn(
                    teacherSurfaceRhythm.cardInset,
                    "grid gap-4 p-5 lg:grid-cols-[minmax(0,1.4fr)_repeat(4,minmax(0,0.7fr))_auto] lg:items-center",
                  )}
                >
                  ...
                  <div>
                    ...
                    <Badge variant={student.tone} className="mt-2 w-fit">
                      {student.status}
                    </Badge>
                  </div>
                  <div className="flex items-center gap-2 lg:justify-end">
                    <Button
                      variant="secondary"
                      className="min-h-10 px-3 text-on-surface-variant shadow-none hover:text-on-surface"
                    >
                      <Pencil className="size-4" aria-hidden />
                    </Button>
                    <Button
                      variant="secondary"
                      className="min-h-10 bg-error-container px-3 text-on-error-container shadow-none hover:bg-error-container/90"
                    >
              ...
              <div className="flex items-center gap-2">
                {[1, 2, 3].map((page) => (
                  <span
                    ...
                    className={
                      page === 1
                        ? "grid size-10 place-items-center rounded-full bg-primary text-on-primary"
                        : "grid size-10 place-items-center rounded-full bg-surface-container-lowest shadow-ambient"
                    }
                  >
                    {page}
                  </span>
                ...
                <span className="grid size-10 place-items-center rounded-full bg-surface-container-lowest shadow-ambient">
                  125
                </span>
              </div>
            </div>
        ...
        <aside className="grid gap-4 self-start">
          {stats.map((stat) => (
            <Card
              key={stat.label}
              className="rounded-[var(--radius-shell)] bg-surface-container-lowest p-5"
            >
              ...
              <p className="mt-3 text-[2rem] font-semibold text-on-surface">
                {stat.value}
              </p>
            ...
            <div className="mt-4 grid gap-3">
              <Button
                ...
                className={cn(
                  ...
                  "min-h-12 justify-between bg-surface-container-lowest px-4 py-4 text-left text-on-surface shadow-none",
                )}
              >
                导出学生名册
